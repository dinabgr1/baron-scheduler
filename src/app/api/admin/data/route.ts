...
import type { Booking, FlightLog, Pilot, Rate, HourPackage, MaintenanceRecord, AircraftSetting } from '@/lib/db'

export const dynamic = 'force-dynamic'

export async function GET() {
  const [bookings, flightLogs, pilots, rates, hourPackages, maintenanceRecords, lastLog, airframeSetting] = await Promise.all([
    ...
    dbFirst<AircraftSetting>("SELECT value FROM aircraft_settings WHERE key = 'airframe_hours_initial'"),
  ])

  const currentHobbs = lastLog?.hobbs_end || 0
  const initial = parseFloat(airframeSetting?.value || '0')
  const flightHoursSum = flightLogs.reduce((sum, log) => sum + (log.flight_time_hours || 0) + (log.flight_time_minutes || 0) / 60, 0)
  const totalAirframeHours = initial + flightHoursSum

  return NextResponse.json({
    bookings, flightLogs, pilots, rates, hourPackages,
    maintenance: { records: maintenanceRecords, currentHobbs, totalAirframeHours },
  }, {
